...
def run(test, params, env):
    ...
    # 2.Mount resctrl
    #process.run("mount -t resctrl resctrl /sys/fs/resctrl",shell=True)
    process.run("echo 'L3:0=0ff;1=0ff' > /sys/fs/resctrl/schemata",shell=True)

    # 3.Check host MBA info from virsh capabilities output
    cmd = "virsh capabilities | awk '/<memory_bandwidth>/,/<\/memory_bandwidth>/'"
    out = ""
    out= to_text(process.system_output(cmd, shell=True))

    if not re.search('node',out):
        raise exceptions.TestFail("There is no memory_bandwidth info in capablities")
    ...
    node_item_list1 = [ast.literal_eval(x)
                       for x in params.get("node_items1","").split()]
    ...
    try:
        ...
        if memorytune_item_list:
            ...
            for i in range(len(memorytune_item_list)):
                ...
                logging.debug("memorytunexml.xmltreefile: %s" % memorytunexml.xmltreefile)
                ...
                for node in node_item_list[i]:
                    ...
                    #logging.debug("before - nodexml.xmltreefile: %s" % nodexml.xmltreefile)
                    nodexml.id = node['id']
                    nodexml.bandwidth = node['bandwidth']
                    #logging.debug("after -nodexml.xmltreefile: %s" % nodexml.xmltreefile)
        ...
        if cachetune_items:
            ...
            for i in range(len(cachetune_item_list)):
                ...
                logging.debug("cachetunexml.xmltreefile: %s" % cachetunexml.xmltreefile)
        ...
        if not found_mb:
            test.fail("schemata %s for vcpus is not set valid" % schemata_file1)
        found_mb = verify_membind_value(schemata_file2, mb_value2)
        if not found_mb:
            test.fail("schemata %s for vcpus is not set valid" % schemata_file2)
        ...
        if os.path.exists(schemata_file1) or os.path.exists(schemata_file2):
            test.fail("the file should be deleted after vm destroy")
